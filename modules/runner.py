...
def run(df_in, var_map_in, set_seed, target, 
        encoding_metric, selection_metric):
    
    df, var_map = DataGetter(df_in, var_map_in).run()
    
    df_train, df_test, performance_summary =\
        Preprocessor(set_seed, target, df, encoding_metric).run()
    
    (logit, ann, knn, svm, rf,
     X_train, X_test, y_train, y_test) =\
        Modeler(set_seed, target, df_train, df_test).run(selection_metric)
    
    Validator(X_train, y_train, X_test, y_test).run([logit, ann, knn,
                                                     svm, rf])
