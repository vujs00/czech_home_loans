...
df_train = pd.read_parquet(interim_library_path + r'\02-2_-_df_train.parquet')
df_smot_train = pd.read_parquet(interim_library_path + r'\02-2_-_df_smot_train.parquet')

df_test = pd.read_parquet(interim_library_path + r'\02-2_-_df_test.parquet')
df_smot_test = pd.read_parquet(interim_library_path + r'\02-2_-_df_smot_test.parquet')
...
logit_model(df_train = df_train, df_test = df_test, 
            sfs_vars_export_path = sfs_vars_export_path, 
            model_output_name = 'logit_summary')

#------------------------------------------------------------#
# STEP 5: smot-expanded dataset                              #
#------------------------------------------------------------#
    
logit_model(df_train = df_smot_train, df_test = df_smot_test, 
